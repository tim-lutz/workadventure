...
<div class="block-container">
    <h3>{Translator.trans("report.block.title")}</h3>
    <p>{Translator.trans("report.block.content", { userName })}</p>
    <button type="button" class="nes-btn is-error" on:click|preventDefault={blockUser}>
        {userIsBlocked ? Translator.trans("report.block.unblock") : Translator.trans("report.block.block")}
    </button>
</div>
