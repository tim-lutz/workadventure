...
<form class="customCharacterScene">
    <section class="text-center">
        <h2>{Translator.trans("custom-character.title")}</h2>
    </section>
    ...
    <section class="action">
        {#if $activeRowStore === 0}
            <button type="submit" class="customCharacterSceneFormBack nes-btn" on:click|preventDefault={previousScene}
                >{Translator.trans("custom-character.navigation.return")}</button
            >
        {/if}
        {#if $activeRowStore !== 0}
            <button type="submit" class="customCharacterSceneFormBack nes-btn" on:click|preventDefault={selectUp}
                >{Translator.trans("custom-character.navigation.back")}
        ...
        {#if $activeRowStore === 5}
            <button
                ...
                on:click|preventDefault={finish}>{Translator.trans("custom-character.navigation.finish")}</button
            >
        {/if}
        {#if $activeRowStore !== 5}
            <button
                ...
                >{Translator.trans("custom-character.navigation.next")}
