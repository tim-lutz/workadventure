...
<form class="selectCompanionScene">
    <section class="text-center">
        <h2>{Translator.trans("companion.select.title")}</h2>
    ...
    <section class="action">
        <button href="/" class="selectCompanionSceneFormBack nes-btn" on:click|preventDefault={noCompanion}
            >{Translator.trans("companion.select.any")}</button
        >
        <button
            type="submit"
            class="selectCompanionSceneFormSubmit nes-btn is-primary"
            on:click|preventDefault={selectCompanion}>{Translator.trans("companion.select.continue")}</button
        >
    </section>
</form>
